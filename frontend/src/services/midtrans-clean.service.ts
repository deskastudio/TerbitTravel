...
export class MidtransService {
  ...
  public static async openPaymentPopup(
    ...
    try {
      ...
      // Create snap token first
      const token = await this.createSnapToken(bookingId);
      ...
      if (!scriptLoaded) {
        throw new Error("Failed to load Midtrans script");
      }
      ...
      // 🔧 FIX: Configure Snap pay with proper error handling
      const snapConfig = {
