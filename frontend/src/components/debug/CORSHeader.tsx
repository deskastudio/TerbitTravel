...
const CORSHeader = () => {
  const [requestHeaders, setRequestHeaders] = useState<RequestHeaders | null>(null);
    ...
    <Card className="w-full mb-4">
      <CardHeader>
        <CardTitle className="flex justify-between items-center">
          ...
          <Button onClick={testCORS} disabled={isLoading} variant="outline" size="sm">
      ...
      <CardContent className="space-y-4 text-sm">
        ...
        {requestHeaders && (
          <div className="space-y-2">
            ...
            <div className="bg-gray-50 p-3 rounded overflow-x-auto">
              <pre className="text-xs">{JSON.stringify(requestHeaders, null, 2)}</pre>
            </div>
          </div>
        )}
        ...
        {responseHeaders && (
          <div className="space-y-2">
            ...
            <div className="bg-gray-50 p-3 rounded overflow-x-auto">
              <pre className="text-xs">{JSON.stringify(responseHeaders, null, 2)}</pre>
            </div>
            ...
            <div className="space-y-1 mt-2">
              ...
              <div className="grid grid-cols-2 gap-1">
                ...
                <div>{responseHeaders['access-control-allow-origin'] || '❌ Missing'}</div>
                
                <div className="font-medium">Access-Control-Allow-Credentials:</div>
                <div>{responseHeaders['access-control-allow-credentials'] || '❌ Missing'}</div>
                
                <div className="font-medium">Access-Control-Allow-Methods:</div>
                <div>{responseHeaders['access-control-allow-methods'] || '❌ Missing'}</div>
                
                <div className="font-medium">Access-Control-Allow-Headers:</div>
                <div className="truncate">
                  {responseHeaders['access-control-allow-headers'] || '❌ Missing'}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-2">
          ...
          <ul className="list-disc list-inside space-y-1 text-sm text-gray-700">
            <li>Browser prevents setting <strong>Origin</strong> header manually (security restriction)</li>
            <li>Server must respond with <strong>Access-Control-Allow-Origin</strong> matching your origin</li>
            <li>For cookies, server must set <strong>Access-Control-Allow-Credentials: true</strong></li>
            <li>Check that backend CORS config includes your frontend origin</li>
          </ul>
        </div>
      </CardContent>
    </Card>
