...
const RegisterPage = () => {
  ...
  const { register: registerUser, googleRegister, login } = useAuth();
  ...
  const onSubmit = async (values: RegisterFormValues) => {
    try {
      ...
      toast({
        title: "Berhasil",
        description: "Registrasi berhasil! Sedang login...",
      });

      // Auto login setelah registrasi berhasil
      try {
        // Hook useAuth login method menerima email dan password terpisah
        const loginResponse = await login(values.email, values.password);

        // Login berhasil, redirect ke home
        toast({
          title: "Selamat Datang!",
          description: "Login berhasil. Selamat datang di Travedia!",
        });
  ...
  const handleGoogleSuccess = async (response: any) => {
    try {
      ...
      toast({
        title: "Berhasil",
        description: "Registrasi dengan Google berhasil! Selamat datang!",
      });

      // Auto redirect ke home setelah Google register berhasil
      setTimeout(() => {
        navigate("/");
      }, 1000);
    } catch (error: any) {
