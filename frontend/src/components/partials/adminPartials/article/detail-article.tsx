      ...
      <div className="flex items-center justify-center h-screen">
        <Card className="w-96 text-center">
          <CardContent className="pt-6">
            ...
            <p className="text-gray-600 mb-4">Artikel yang Anda cari tidak tersedia atau telah dihapus.</p>
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbPage className="text-gray-600">Detail Artikel</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2">
            <Badge variant="secondary" className="flex items-center gap-1">
              ...
              {typeof article.penulis === 'string' ? article.penulis : article.penulis?.nama || 'Unknown'}
            </Badge>
            <Badge variant="outline" className="flex items-center gap-1">
              <Tags className="w-3 h-3 text-blue-500" />
              {getCategoryName(article.category || (Array.isArray(article.kategori) ? article.kategori[0] : article.kategori))}
            </Badge>
            ...
            <div className="aspect-[16/9] bg-gray-100">
              {article.gambarUtama || article.gambar ? (
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src = 'https://placehold.co/800x450?text=Gambar+Artikel';
                  }}
                />
              ) : (
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-4">
                <div className="text-center p-3 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{getWordCount(article.isi)}</div>
                  <div className="text-sm text-gray-600">Total Kata</div>
                </div>
                <div className="text-center p-3 bg-green-50 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{article.isi.length}</div>
                  <div className="text-sm text-gray-600">Total Karakter</div>
                </div>
                <div className="text-center p-3 bg-purple-50 rounded-lg">
                  <div className="text-2xl font-bold text-purple-600">{getReadingTime(article.isi)}</div>
