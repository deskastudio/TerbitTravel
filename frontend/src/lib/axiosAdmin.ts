...
adminAxiosInstance.interceptors.request.use(
  (config) => {
    ...
    if (config.data) {
      console.log(`📤 Request Data:`, typeof config.data === 'string' ? config.data : JSON.stringify(config.data, null, 2));
    }
    ...
    console.log(`📋 Request Headers:`, {
      ...
      'ngrok-skip-browser-warning': config.headers['ngrok-skip-browser-warning'] || 'Not set'
    });
...
adminAxiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    const errorInfo = {
      ...
      isCrossOrigin: error.config?.baseURL ? !error.config.baseURL.includes(window.location.origin) : false
    };
