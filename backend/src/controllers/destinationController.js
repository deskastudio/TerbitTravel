  ...
  try {
      ...
      for (const imageToDelete of imagesToDelete) {
        ...
        try {
          if (fs.existsSync(filePath)) {
            ...
            console.log(`✅ Successfully deleted individual image: ${imageToDelete}`);
          } else {
            console.log(`⚠️ Individual image file not found: ${imageToDelete}`);
          }
        } catch (error) {
          console.error(`❌ Error deleting individual image ${imageToDelete}:`, error);
        }
        
        // Remove from destination.foto array
        destination.foto = destination.foto.filter(foto => foto !== imageToDelete);
      }
    ...
    if (fotoPaths.length > 0) {
      ...
      if (replaceImages === "true") {
        console.log("🔄 REPLACE MODE: Deleting old images and setting new ones");
