...
function updateBackendEnv() {
  ...
  if (frontendEnv) {
    ...
    if (frontendUrlMatch && frontendUrlMatch[1] && !origins.includes(frontendUrlMatch[1])) {
    ...
    if (apiBaseUrlMatch && apiBaseUrlMatch[1] && !origins.includes(apiBaseUrlMatch[1])) {
    ...
    if (originsMatch) {
      updatedEnv = updatedEnv.replace(/ALLOWED_ORIGINS=([^\n]*)/, `ALLOWED_ORIGINS=${uniqueOrigins.join(',')}`);
    } else {
      updatedEnv += `\nALLOWED_ORIGINS=${uniqueOrigins.join(',')}`;
    }
  }
...
if (success) {
  ...
  console.log('   2. Jalankan tunnel di terminal terpisah dengan "npm run tunnel"');
} else {
