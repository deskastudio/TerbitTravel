...
function updateBackendEnv(tunnelUrl) {
  ...
  if (envContent.includes('LOCALTUNNEL_URL=')) {
    envContent = envContent.replace(/LOCALTUNNEL_URL=.*/g, `LOCALTUNNEL_URL=${tunnelUrl}`);
  } else {
...
function updateEnv(tunnelUrl) {
  ...
  if (frontendUpdated) {
    ...
    console.log(`   ⚠️ Frontend .env tidak diupdate (file tidak ditemukan atau tidak ada variabel yang diupdate)`);
  }
}
...
const waitForServer = async (port, maxRetries = 30) => {
  console.log(`🔍 Menunggu server di port ${port} siap sebelum membuat tunnel...`);
  ...
  while (retries < maxRetries) {
    try {
      console.log(`⏳ Menunggu server startup... (Percobaan ${retries + 1}/${maxRetries})`);
...
ltProcess.stdout.on('data', (data) => {
  ...
  if (urlMatch && urlMatch[1]) {
    ...
    console.log(`   Backend dan frontend sekarang menggunakan URL: ${tunnelUrl}`);
  }
});
...
ltProcess.stderr.on('data', (data) => {
  ...
  if (errorMsg.includes('domain in use') || errorMsg.includes('unavailable')) {
    ...
    if (isWindows) {
      ...
      const newLtProcess = spawn('cmd', ['/c', command], { stdio: 'inherit' });
      
      // Replace the current process
      ltProcess = newLtProcess;
    } else {
      const newLtProcess = spawn('npx', ['lt', '--port', PORT, '--subdomain', newSubdomain], { stdio: 'inherit' });
...
ltProcess.on('close', (code) => {
  if (code !== 0) {
    ...
    console.log(`💡 Jika mengalami masalah dengan subdomain, coba gunakan subdomain lain:`);
    console.log(`   npm run tunnel -- --subdomain nama-subdomain-lain`);
  } else {
    console.log(`⛔ localtunnel process berhenti dengan kode ${code}`);
  }
