...
function readEnvFile(filePath) {
  try {
    ...
    console.error(
      chalk.red(`❌ Error membaca file ${filePath}: ${error.message}`)
    );
    return null;
  }
}
...
function displayInfo(backendContent, frontendContent) {
  ...
  const allUrlsAllowed = frontendUrls.every((url) =>
    backendOrigins.includes(url)
  );

  if (allUrlsAllowed) {
    console.log(
      chalk.green(
        "  ✅ Semua URL frontend terdaftar di ALLOWED_ORIGINS backend"
      )
    );
  } else {
    console.log(
      chalk.red(
        "  ❌ Beberapa URL frontend tidak terdaftar di ALLOWED_ORIGINS backend:"
      )
    );
  ...
  // Additional checks
  const hasHttpsBackend = backendOrigins.some((url) =>
  ...
  if (hasHttpsBackend !== hasHttpsOrigin) {
    console.log(
      chalk.yellow(
        "  ⚠️ Perhatian: Mixing HTTP dan HTTPS dapat menyebabkan masalah CORS"
      )
    );
  }
  ...
  if (!allUrlsAllowed) {
    console.log(
      chalk.yellow(
        "  - Tambahkan semua URL frontend ke ALLOWED_ORIGINS di backend"
      )
    );
  }
  ...
  console.log(
    chalk.yellow(
      "  - Pastikan withCredentials: true di axios jika menggunakan cookies"
    )
  );
  ...
  console.log(
    chalk.white("  2. Dalam tab terminal lain, jalankan: npm run tunnel")
  );
